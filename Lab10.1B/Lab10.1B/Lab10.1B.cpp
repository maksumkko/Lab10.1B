...
void count_good_grades(Students* S, int N, int& count_math, int& count_physics, int& count_program, int& count_nm, int& count_ped, int& count_physics_5);
...
int main()
{
    ...
    cout << "Кількість студентів, які отримали оцінку '5' з фізики: " << count_physics_5 << endl;

    cout << "\nСтуденти без трійок (ті, хто має оцінки не нижче 'добре'):" << endl;
...
{
    for (int i = 0; i < N; i++) {
        if (S[i].grade_physics >= 4 && S[i].grade_math >= 4) {
            switch (S[i].speciality)
            {
            case KN:
                if (S[i].grade_program >= 4) {
